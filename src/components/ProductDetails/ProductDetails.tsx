...
export default function ProductDetails() {
    ...
    const { product, isLoading, error } = useAppSelector((state) => state.details);
    const dispatch = useAppDispatch();

    const decrement = () => setCountProduct((count) => {
        count -= 1
        if (count < 0) return 0
        return count;
    })

    const increment = () => setCountProduct((count) => {
        count += 1
    ...
    return (
        <>
            {isLoading && <Loader />}
            {error && <ErrorWidget text={error} request={() => id && dispatch(getProductDetails(id))} />}
            {product &&
                ...
                <div className="row">
                    <div className="col-5">
                        <img
                            ...
                            className="img-fluid" alt={`Изображение товара: ${product.title}`}
                        />
                    </div>
                    <div className="col-7">
                        ...
                        <>
                            <div className="text-center">
                                <SizesSelect active={sizeSelected} changeActive={setSizeSelected} sizesList={product.sizes} />
                                <p>Количество: <span className="btn-group btn-group-sm pl-2">
                                        <ButtonChangeCount title={'-'} setCount={decrement}  />
                                        <span className="btn btn-outline-primary">{countProduct}</span>
