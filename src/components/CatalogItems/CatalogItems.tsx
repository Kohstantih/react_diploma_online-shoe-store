...
export default function CatalogItems() {
  const { isLoading, items, offset, error } = useAppSelector(
    (state) => state.catalog
  );
    ...
    <>
      ...
      {offset !== 0 && !isLoading && !error && (
        <div className="text-center">
          <button
            onClick={() => {
              dispatch(
                dispatch(getCatalogItemsMore(offset, activeCategoryId, search))
              );
            }}
      ...
      {error && (
        <ErrorWidget
          ...
          request={() => {
            if (search)
              dispatch(getSearchResult(search, activeCategoryId, offset));
            else dispatch(getCatalogItems(activeCategoryId));
          }}
        />
      )}
