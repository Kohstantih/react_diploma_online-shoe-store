...
import { failureLoadCatalogItems, startLoadingCatalogItemsMore, successLoadCatalogItemsMore } from '../reducers/setCatalogItemSlice';

import { TCardObject } from '../../types/TCardObject';
import { TGetCatalogItemsMoreArgs } from '../../types/TGetCatalogItemsMoreArgs';

function *handleGetCatalogItemsMore(action: PayloadAction<TGetCatalogItemsMoreArgs>) {
    yield put(startLoadingCatalogItemsMore());

    try {
        const retryDelay = 2 * 1000;
        const data: TCardObject[] = yield retry(3, retryDelay, fetchCatalogItemsMore, action.payload);
